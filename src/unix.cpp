...
static int unix_stream_socket_write(lua_State* L)
{
    lua_settop(L, 2);
...
void init_unix(lua_State* L)
{
    ...
    lua_pushlightuserdata(L, &unix_stream_socket_write_key);
    res = luaL_loadbuffer(L, reinterpret_cast<char*>(data_op_bytecode),
                          data_op_bytecode_size, nullptr);
    assert(res == 0); boost::ignore_unused(res);
    rawgetp(L, LUA_REGISTRYINDEX, &raw_error_key);
    lua_pushcfunction(L, unix_stream_socket_write);
    lua_call(L, 2, 1);
    lua_rawset(L, LUA_REGISTRYINDEX);
