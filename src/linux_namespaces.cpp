...
static int child_main(void*)
{
...

    {
        ...
        sa.sa_flags = 0;
        if (sigaction(SIGCHLD, &sa, /*oldact=*/NULL) == -1)
            return 1;
    ...
    if (getpid() == 1) {
        ...
        default: {
            ...
            if (sigaction(SIGTERM, &sa, /*oldact=*/NULL) == -1)
                return 1;
            if (sigaction(SIGUSR1, &sa, /*oldact=*/NULL) == -1)
                return 1;
            if (sigaction(SIGUSR2, &sa, /*oldact=*/NULL) == -1)
                return 1;

            // Applications that don't require a controlling terminal, such as
            // daemons, usually re-purpose SIGHUP as a signal to re-read
            // configuration files.
            if (sigaction(SIGHUP, &sa, /*oldact=*/NULL) == -1)
                return 1;
            ...
            // SIGINT to PID1.
            if (sigaction(SIGINT, &sa, /*oldact=*/NULL) == -1)
                return 1;
