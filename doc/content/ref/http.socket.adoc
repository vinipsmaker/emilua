...
s:connect(r:resolve('www.example.com', '')[1].ep_addr, 80)
s = http.socket.new(s)
...
to `"body-ready"` once all body was read. In streaming connections
...
`"body-ready"`::
This state is reached from the `"message-ready"`, once the http producer (e.g.
...
+
It can be reached from `"body-ready"` state, after all trailers have been
...
+
Another interpretation is that it was reached from the `"body-ready"` state
...
`"body-ready"`::
This state is reached from the `"message-ready"`, once the http producer
...
`"metadata-issued"`::
This state can be reached either from `"empty"` or `"continue-issued"`.
+
