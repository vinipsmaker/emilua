...
The fiber will immediately suspend until the error for the current cleanup
handler is taken care of. On ``join()``ed fibers, the fiber handle will remain
joinable so you can further resume the action of the remaining cleanup
handlers. On ``detach()``ed fibers, the VM is terminated (there's no way to
recover from this error without context, and conceptually by the time uncaught
hooks are executed, the context was already lost).

For the common case, you shouldn't try to `join()` the fiber a second time
trying to prevent VM termination on cleanup errors. Just let the handle be GC'ed
to terminate the VM. Proper recovery in these cases is pretty tricky and your
best bet is to change the code and fix the error in the cleanup handler. If you
need some sort of protection against one complex module that will fail now and
then, run it in a separate actor.
--
