...
* Pending operations must not trust `L` from the initiating operation to decide
  ...
  template.
* Don't forget to call `lua_checkstack()` before resuming a suspended fiber (if
  there is any value that you need to push to the stack). Lua only allocates
  `LUA_MINSTACK` when the VM calls a C function. Therefore you don't have
  `LUA_MINSTACK` when you're resuming a Lua thread. There are non-obvious paths
  that executed Lua code can walk to reach this limit.
